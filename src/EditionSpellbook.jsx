...
import {characterSlots,spellSlotPools,spellCounts,castingKey,is35,mechanics,permittedSpells,keyOf,resolveSpell,editionName} from './lib/editions';
...
export default function EditionSpellbook({char,patch,roll,show,homebrew=[]}) {
  ...
  const perClassLegacySlots=multiple&&is35(classModel),activeSlotsUsed=perClassLegacySlots?(char.classSlotsUsed?.[active.catalogId]||{}):(char.slotsUsed||{});
  const slotCharacter=perClassLegacySlots?{...classModel,slotsUsed:activeSlotsUsed}:char;
  ...
  const candidates=permittedSpells(char.ruleset==='custom'?{...classModel,ruleset:'custom',mechanics:char.mechanics}:classModel,[...homebrew,...reference.entries]);
  ...
  const cantrips=classSpells.filter(s=>!s.auto&&s.level===0),leveled=classSpells.filter(s=>!s.auto&&s.level>0);
  ...
  function add(s) {
    ...
    const number=classSpells.filter(x=>!x.auto&&(x.level===0)===(s.level===0)).length;
    ...
    if(number>=limit)return;
    patch({spells:[...char.spells,{...s,id:crypto.randomUUID(),castingClassId:multiple?active.catalogId:s.castingClassId,prepared:manual||counts.mode!=='spellbook'}]});
  }
  function open(s){const data={...resolveSpell(s,char),...s};setCast(data);setCastError('');const first=slotOptions(data)[0];setSlot(first?.level??Math.max(is35(char)?0:1,Number(data.level)||0));setSlotPool(first?.pool||'standard');setFormula(s.rollFormula||'');setAttack(!!s.rollAttack);}
