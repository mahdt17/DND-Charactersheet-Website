...
export default function EditionSpellbook({char,patch,roll,show,homebrew=[]}) {
  ...
  const slots=characterSlots(char),ability=castingKey(classModel),mod=modifier(effectiveAbilities(char)[ability]||10);
  ...
  return <><div className="l-section-head"><h2>Spellbook · {editionName(char.ruleset)}</h2><button className="l-button" onClick={()=>setManage(!manage)}>{manage?'Done':'Manage spells'}</button></div>
    ...
    {(manual||multiple)&&<details className="feature-detail"><summary>Spell slots and homebrew adjustments</summary><p>Set total slots from the applicable spellcasting / multiclass rules. Each class’s spell choices use its individual level. Pact Magic, prestige advancement and cross-edition pools require an explicit table ruling here. In 3.5, level 0 spells also consume slots; damage scales with caster level. Psionic power points use an Actions resource counter.</p><div className="form-grid">{slots.map((n,i)=><label className="l-field" key={i}><span>Level {i} slots</span><input type="number" min="0" max="30" value={n} onChange={e=>patch({slotOverride:slots.map((x,j)=>j===i?Math.max(0,Math.min(30,Number(e.target.value)||0)):x)})}/></label>)}</div></details>}
