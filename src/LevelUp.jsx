...
export default function LevelUp({char,homebrew=[],onFinish,onCancel}) {
  ...
  const nextClassLevel=(row?.level||0)+1,castingPlan=candidate?castingAdvancementPlan(eligibilityCharacter,candidate,nextClassLevel):{groups:[],valid:true};
  const effectiveCastingTargets=Object.fromEntries(castingPlan.groups.map(group=>[group.id,castingTargets[group.id]||(group.candidates.length===1?group.candidates[0].classId:'')]).filter(([,value])=>value));
  const training=multiclassTrainingPlan(eligibilityCharacter,candidate);
  function changeFlow(value){setFlow(value);setSelected('');setConfirmations({});setReviewed(false);setTrainingChoices({});setInheritanceChoice('');setCastingTargets({});}
  ...
  return <Dialog title="Choose how to level up" wide onClose={onCancel}>
    ...
    {castingPlan.groups.length>0&&<section aria-label="Spellcasting advancement"><h3>Spellcasting / manifesting advancement</h3><p>This class level advances existing casting or manifesting rather than creating a separate slot pool. Choose the target class for each source-defined advancement.</p>{castingPlan.groups.map(group=><label className="l-field" key={group.id}><span>{group.kind} · {group.label}</span><select value={effectiveCastingTargets[group.id]||''} onChange={e=>setCastingTargets({...castingTargets,[group.id]:e.target.value})}><option value="">Choose existing class</option>{group.candidates.map(option=><option key={option.classId} value={option.classId}>{option.name} {characterClasses(char).find(row=>row.catalogId===option.classId)?.level||''}</option>)}</select>{!group.candidates.length&&<small>No compatible existing class is currently available; review the prerequisite/source combination.</small>}</label>)}</section>}
    ...
    <div className="l-toolbar"><button className="l-button" onClick={onCancel}>Cancel</button><button className="l-button primary" disabled={!result.allowed||candidate?.inheritanceRequired||!castingAdvancementSelectionsValid(castingPlan,effectiveCastingTargets)||!trainingChoicesValid(training,trainingChoices)||((!row||rows.length>1)&&!reviewed)||catalog.loading} onClick={()=>setProceed(true)}>Continue to level choices</button></div>
  </Dialog>;
}
