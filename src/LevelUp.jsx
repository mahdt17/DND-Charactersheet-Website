...
export default function LevelUp({char,homebrew=[],onFinish,onCancel}) {
  ...
  function changeFlow(value){setFlow(value);setSelected('');setConfirmations({});setReviewed(false);setTrainingChoices({});}
  ...
  return <Dialog title="Choose how to level up" wide onClose={onCancel}>
    ...
    {training.mode==='automatic'&&<section aria-label="Multiclass proficiencies"><h3>Multiclass proficiencies</h3><p>{training.grants.length?`Added on level-up: ${training.grants.map(p=>p.name).join(', ')}.`:'This class grants no fixed multiclass proficiencies.'}</p>{training.choices.map(c=>c.required?<label className="l-field" key={c.id}><span>{c.label}</span><select aria-label={c.label} value={trainingChoices[c.id]||''} onChange={e=>setTrainingChoices({...trainingChoices,[c.id]:e.target.value})}><option value="">Choose one untrained option</option>{c.options.map(n=><option key={n}>{n}</option>)}</select></label>:<p key={c.id}>You already have every listed {c.kind==='skills'?'skill':'instrument'} proficiency. No additional choice is available.</p>)}</section>}
    ...
    <div className="l-toolbar"><button className="l-button" onClick={onCancel}>Cancel</button><button className="l-button primary" disabled={!result.allowed||!trainingChoicesValid(training,trainingChoices)||((!row||rows.length>1)&&!reviewed)||catalog.loading} onClick={()=>setProceed(true)}>Continue to level choices</button></div>
  </Dialog>;
}
