...
export default function TrainingPanel({char,patch,abilities}) {
  ...
  return <section aria-label="Training and proficiencies">
    ...
    {grants.length>0&&<><h3>Training grants</h3>{[['armor','Armor'],['weapons','Weapons'],['tools','Tools'],['skills','Skills']].map(([kind,label])=>{
      const names=grants.filter(p=>p.kind===kind).map(p=>p.name);
      return names.length?<p key={kind}><strong>{label}:</strong> {names.join(', ')}</p>:null;
    })}</>}
    {Object.values(char.featureChoices||{}).length>0&&<><h3>Recorded feature choices</h3>{Object.entries(char.featureChoices).map(([id,g])=><p key={id}><strong>{g.className} {g.level} · {g.feature}:</strong> {g.choices.join(', ')||'No eligible choices remained'}</p>)}</>}
    {!!char.toolExpertise?.['thieves-tools']&&<p><strong>Thieves’ tools:</strong> Expertise · double your proficiency bonus on applicable tool checks.</p>}
