    ...
    <div className="creation-shell">
      ...
      <section className="creation-content">
        ...
        <div className="creation-scroll">
          ...
          {step===spellStep&&<><CreationStepHeader eyebrow="Spellcasting" title={stepPrompt[6][0]} description={stepPrompt[6][1]}/>
            {starterSpellCounts?.cantrips>0&&<SpellPicker title="Starting cantrips" spells={availableSpells(draft.className,0,true)} selected={draft.spellChoices.cantrips} limit={starterSpellCounts.cantrips} onToggle={name=>setDraft(d=>{const list=d.spellChoices.cantrips;return {...d,spellChoices:{...d.spellChoices,cantrips:list.includes(name)?list.filter(x=>x!==name):list.length<starterSpellCounts.cantrips?[...list,name]:list}};})}/>}
            {starterSpellLimit>0&&<SpellPicker title="Starting spells" spells={availableSpells(draft.className,1,false).filter(s=>s.level>0)} selected={draft.spellChoices.spells} limit={starterSpellLimit} onToggle={name=>setDraft(d=>{const list=d.spellChoices.spells;return {...d,spellChoices:{...d.spellChoices,spells:list.includes(name)?list.filter(x=>x!==name):list.length<starterSpellLimit?[...list,name]:list}};})}/>}
            <div className="creation-auto-note"><strong>Added automatically</strong><span>Your selected spells appear on the character sheet when you finish.</span></div></>}
