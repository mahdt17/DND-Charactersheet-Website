...
try {
 ...
 assert(await page.getByLabel('Current temporary HP',{exact:true}).isVisible());await page.getByLabel('Current temporary HP').fill('8');await page.getByLabel('Temporary HP options',{exact:true}).click();await page.getByLabel('Temporary HP adjustment').fill('5');await page.getByRole('button',{name:'Grant temp HP',exact:true}).click();assert.equal(await page.getByLabel('Current temporary HP').inputValue(),'8');await page.getByRole('button',{name:'Reduce temp HP',exact:true}).click();assert.equal(await page.getByLabel('Current temporary HP').inputValue(),'3');await page.getByLabel('Hit point adjustment').fill('5');await page.getByRole('button',{name:'Damage',exact:true}).click();assert.match(await page.locator('.hp-value').innerText(),/^15/);await page.getByLabel('Current temporary HP').fill('7');await page.getByRole('button',{name:'Clear temp HP',exact:true}).click();assert.equal(await page.getByLabel('Current temporary HP').inputValue(),'0');
 await page.getByLabel('Current temporary HP').fill('9');
