...
try {
 ...
 await page.getByRole('button',{name:'Exit demo',exact:true}).click();await page.getByRole('button',{name:'Explore the demo'}).click();assert.equal(await page.getByRole('button',{name:'Open character'}).count(),1);assert.deepEqual(errors,[]);
 const failed=await browser.newPage();await failed.route('**/assets/LedgerEntry-*.js',route=>route.abort());await failed.goto(`http://127.0.0.1:5180${base}`);await failed.getByRole('button',{name:'Explore the demo'}).click();await failed.getByRole('button',{name:'Reload ledger'}).waitFor();assert.match(await failed.getByRole('alert').innerText(),/could not load/);
