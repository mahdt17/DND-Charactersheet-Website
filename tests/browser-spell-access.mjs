...
try {
 ...
 const life=row('Cleric','2014',3);life.subclass='Life';
 await importChar({...make('2014',[life]),name:'Domain spells'});
 assert.equal(await page.locator('.spell-item').count(),4);assert.equal(await page.locator('.spell-item').getByRole('button',{name:'Prepare',exact:true}).count(),0);
 await page.locator('.spell-item').filter({hasText:'Bless'}).getByRole('button',{name:'Cast',exact:true}).click();await page.getByRole('button',{name:'Cast & spend slot',exact:true}).click();
 assert.match(await page.getByRole('button',{name:'Level 1 slot 1, used',exact:true}).getAttribute('class'),/used/);
 await page.getByRole('button',{name:'Level up',exact:true}).click();await page.getByRole('button',{name:'Continue to level choices'}).click();
 await page.locator('.creation-footer').getByRole('button',{name:'Continue',exact:true}).click();await page.locator('.creation-footer').getByRole('button',{name:'Continue',exact:true}).click();
 const normalPrepared=page.getByRole('region',{name:'Prepared spells',exact:true});assert.match(await normalPrepared.innerText(),/0 \/ 8 selected/);
 await normalPrepared.getByLabel('Search spells',{exact:true}).fill('Bless');assert.equal(await normalPrepared.getByRole('button',{name:'Select Bless',exact:true}).count(),0);await page.getByRole('button',{name:'Cancel',exact:true}).click();await page.getByRole('button',{name:'Cancel',exact:true}).click();
 const fiend=row('Warlock','2014',4);fiend.subclass='Fiend';
 const warlockSpells=spells14.filter(s=>['Eldritch Blast','Mage Hand','Minor Illusion','Armor of Agathys','Charm Person','Comprehend Languages','Hellish Rebuke','Hex'].includes(s.name)).map(s=>({...s,id:s.index,edition:'2014',castingClassId:fiend.catalogId}));
 await importChar({...make('2014',[fiend]),name:'Fiend spell choices',spells:warlockSpells});await page.getByRole('button',{name:'Level up',exact:true}).click();await page.getByRole('button',{name:'Continue to level choices'}).click();await page.locator('.creation-footer').getByRole('button',{name:'Continue',exact:true}).click();
 const newKnown=page.getByRole('region',{name:'New spells',exact:true});await newKnown.getByLabel('Search spells',{exact:true}).fill('Fireball');await newKnown.getByRole('button',{name:'Select Fireball',exact:true}).waitFor();await page.getByRole('button',{name:'Cancel',exact:true}).click();await page.getByRole('button',{name:'Cancel',exact:true}).click();
 const land=row('Druid','2024',3);land.subclass='Circle of the Land';
 await importChar({...make('2024',[land]),name:'Circle spells',subclass:'Circle of the Land'});
 await page.getByLabel('Circle of the Land terrain').selectOption('arid');assert.equal(await page.locator('.spell-item').count(),3);assert.equal(await page.locator('.spell-item').filter({hasText:'Fire Bolt'}).count(),1);
 await page.getByLabel('Circle of the Land terrain').selectOption('polar');assert.equal(await page.locator('.spell-item').filter({hasText:'Fire Bolt'}).count(),0);assert.equal(await page.locator('.spell-item').filter({hasText:'Ray of Frost'}).count(),1);
 await page.getByRole('button',{name:'Manage spells',exact:true}).click();await picker('2024').getByLabel('Search spells',{exact:true}).fill('Healing Word');await picker('2024').getByRole('button',{name:/^Select Healing Word/}).click();
 assert.equal(await page.locator('.spell-item').count(),4);assert.equal(await page.getByRole('alert').count(),0);
